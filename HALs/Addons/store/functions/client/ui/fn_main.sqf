	...
	case ("combobox"): {
		...
		switch (toLower _mode) do {
			...
			case ("update"): {
				...
				private _containers = [];
				if (["CONTAINERED_SHOP" , str _trader] call BIS_fnc_inString) then {
					private _syncedContainer = (synchronizedObjects _trader) select 0;

					_containers = [
						[typeOf _syncedContainer, format ["%1 (%2m)", getText(configFile >> "cfgVehicles" >> typeOf _syncedContainer >> "displayName"), (_syncedContainer distance2D _trader) toFixed 0], "", _syncedContainer]	
					];
				} else {
					_containers = [
						[
